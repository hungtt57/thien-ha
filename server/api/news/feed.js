...
function atom_1_0(options) {
    ...
    for(var i = 0; i < entries.length; i++) {
        ...
        if(entries[i].description)
            entry.push({ summary: entries[i].description});

        if(entries[i].content)
            entry.push({ content: entries[i].content});
...
function rss_2_0(options) {
    ...
    for(var i = 0; i < items.length; i++) {
        ...
        if(items[i].description)
            item.push({ description: items[i].description});

        if(items[i].content) {
            bContent = true;
            item.push({ 'content:encoded': items[i].content});
        }
        /**
         * Item Author
